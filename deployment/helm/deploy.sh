...
export EDC_VAULT_NAME="${EDC_NAMESPACE}-vault"
...
if [[ $EDC_DISABLE_HASHI_VAULT ]] ; then
  ...
  # Initialize Vault
  kubectl exec -n "${EDC_NAMESPACE}" -it "${EDC_VAULT_NAME}-0" -- vault operator init -key-shares=1 -key-threshold=1 -format=json > "${EDC_VAULT_NAME}-keys.json"

  EDC_VAULT_ROOT_TOKEN=$(cat "${EDC_NAMESPACE}-vault-keys.json" | jq -r '.root_token')
  export EDC_VAULT_URL="http://${EDC_VAULT_NAME}:8200"

  # Unseal Vault
  kubectl exec -n "${EDC_NAMESPACE}" -it "${EDC_VAULT_NAME}-0" -- vault operator unseal $(jq -r ".unseal_keys_b64[]" "${EDC_VAULT_NAME}-keys.json")

  # Login to Vault
  kubectl exec -n "${EDC_NAMESPACE}" -it "${EDC_VAULT_NAME}-0" -- vault login $(jq -r ".root_token" "${EDC_VAULT_NAME}-keys.json")
...
kubectl -n "${EDC_NAMESPACE}" create secret docker-registry "${EDC_IMAGE_PULL_SECRET_NAME}" \
