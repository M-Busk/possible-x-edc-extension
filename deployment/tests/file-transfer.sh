...
PROVIDER_HEALTH_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" https://$PROVIDER_IP/api/check/health -m $REQUEST_TIMEOUT)
CONSUMER_HEALTH_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" https://$CONSUMER_IP/api/check/health -m $REQUEST_TIMEOUT)
...
CREATE_ASSET_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X POST https://$PROVIDER_IP/management/v2/assets \
...
CREATE_POLICY_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X POST https://$PROVIDER_IP/management/v2/policydefinitions \
  -H 'Content-Type: application/json' \
  -H "X-API-Key: password" \
  -d '{
    "@context": {
...
CREATE_CONTRACTOFFER_RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -X POST https://$PROVIDER_IP/management/v2/contractdefinitions \
  -H 'Content-Type: application/json' \
  -H "X-API-Key: password" \
  -d '{
    "@context": {
...
FETCH_CATALOG_RESPONSE=$(curl -s -X POST https://$CONSUMER_IP/management/v2/catalog/request \
  -H 'Content-Type: application/json' \
  -H "X-API-Key: password" \
  -d '{
    "@context": {
    ...
    "providerUrl": "https://'"$PROVIDER_IP"'/protocol",
...
OFFER_ID=$(echo $CATALOG_CONTRACTOFFER | jq -r '.["odrl:hasPolicy"] | .["@id"]' || echo $CATALOG_CONTRACTOFFER | jq -r '.["odrl:hasPolicy"][0] | .["@id"]')
echo "Negotiating the contract with OFFER_ID=\"$OFFER_ID\" ..."

contract_negotiations_response=$(curl -s -X POST https://$CONSUMER_IP/management/v2/contractnegotiations \
  -H 'Content-Type: application/json' \
  -H "X-API-Key: password" \
  -d '{
    "@context": {
    ...
    "edc:connectorAddress": "https://'"$PROVIDER_IP"'/protocol",
...
for ((i=0; i<$MAX_RETRIES; i++)); do
  ...
  GET_CONTRACT_AGREEMENT_RESPONSE=$(curl -s https://$CONSUMER_IP/management/v2/contractnegotiations/$contract_negotiations_id \
    -H 'Content-Type: application/json' \
    -H "X-API-Key: password" \
    -m $REQUEST_TIMEOUT)
...
file_transfer_response=$(curl -s -X POST https://$CONSUMER_IP/management/v2/transferprocesses \
  -H 'Content-Type: application/json' \
  -H "X-API-Key: password" \
  -d '{
    "@context": {
...
for ((i=0; i<$MAX_RETRIES; i++)); do
  ...
  GET_TRANSFER_PROCESS_RESPONSE=$(curl -s https://$CONSUMER_IP/management/v2/transferprocesses/$TRANSFER_PROCESS_ID \
    -H 'Content-Type: application/json' \
    -H "X-API-Key: password" \
    -m $REQUEST_TIMEOUT)
